"""Stdio MCP bridge for Argus's Hoard.
...
app itself (`python -m argus`, detached, on the port of ARGUS_URL) and waits
for it, so a workspace that starts before the app still gets its tools;
ARGUS_BRIDGE_AUTOSTART=0 turns that off.
"""
...
BASE_URL = os.environ.get("ARGUS_URL", "http://127.0.0.1:5183").rstrip("/")
TOKEN_FILE = Path(
    os.environ.get("ARGUS_TOKEN_FILE")
    or Path(os.environ.get("ARGUS_DATA_DIR") or ROOT / "data") / "mcp-token"
)
NOT_RUNNING = "Open Argus's Hoard (python -m argus) so the assistant can reach your screen history."


def _healthy() -> bool:
    try:
        response = httpx.get(f"{BASE_URL}/api/health", timeout=4, trust_env=False)
        return response.status_code == 200 and response.json().get("service") == "argus-hoard"
    except Exception:
        return False
...
def ensure_running(timeout_s: float = 45.0) -> bool:
    ...
    if _healthy():
        return True
    if os.environ.get("ARGUS_BRIDGE_AUTOSTART", "1") == "0":
        return False
    port = urlparse(BASE_URL).port or 5183
    env = {**os.environ, "ARGUS_PORT": str(port), "PORT_STRICT": "1", "PYTHONUNBUFFERED": "1"}
    ...
    logs = Path(os.environ.get("ARGUS_DATA_DIR") or ROOT / "data") / "logs"
    logs.mkdir(parents=True, exist_ok=True)
    with open(logs / "argus-app.log", "ab") as out:
        subprocess.Popen([sys.executable, "-m", "argus"], cwd=ROOT, env=env, stdin=subprocess.DEVNULL,
...
class ArgusBridge(FastMCP):
    ...
    def __init__(self, catalog: list[dict], instructions: str):
        super().__init__(name="argus-hoard", instructions=instructions)
...
def main() -> None:
    ...
    ArgusBridge(catalog, instructions).run(transport="stdio")
