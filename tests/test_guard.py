...
from argus.guard import check_request, host_of, install_guard, is_allowed_host, parse_allowed_hosts
...
def test_check_request_fetch_metadata_rules():
    local = {"host": "localhost:5183"}
    assert check_request("GET", local) is None  # curl / MCP bridge: no Sec-Fetch headers
    assert check_request("POST", {"host": "127.0.0.1:5183"}) is None
