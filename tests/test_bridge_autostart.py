...
def load(monkeypatch, autostart: str):
    monkeypatch.setenv("ARGUS_URL", "http://127.0.0.1:1")
    monkeypatch.setenv("ARGUS_BRIDGE_AUTOSTART", autostart)
    spec = importlib.util.spec_from_file_location("argus_bridge_under_test", ROOT / "mcp_server.py")
...
def test_nothing_answers_so_the_bridge_starts_the_app(monkeypatch, tmp_path):
    ...
    argv, env = started[0]
    assert argv == [sys.executable, "-m", "argus"]
    assert env["ARGUS_PORT"] == "1" and env["PORT_STRICT"] == "1"
